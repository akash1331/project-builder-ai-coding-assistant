"""Pydantic models that define the structured data passed between agents.

These schemas are used as ``with_structured_output`` targets so the LLM
returns validated objects (plans, tasks, coder state) instead of free text.
"""
...
class File(BaseModel):
    """A single file the project should contain, with its intended purpose."""
...
class Plan(BaseModel):
    """High-level project blueprint produced by the planner agent."""
...
class ImplementationTask(BaseModel):
    """A single, self-contained coding step targeting one file."""
...
class TaskPlan(BaseModel):
    """Ordered list of implementation tasks produced by the architect agent."""
    implementation_steps: list[ImplementationTask] = Field(description="A list of steps to be taken to implement the task")
    model_config = ConfigDict(extra="allow")

class CoderState(BaseModel):
    """Mutable state tracking the coder agent's progress through the task plan."""
